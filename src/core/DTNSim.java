...
//					firstConfIndex = 2;
	...
	private static void initSettings(String[] confFiles, int firstIndex) {
		...
		try {
			...
			System.err.println("Warning: using deprecated way of " + "expressing run indexes. Run index should be the " + "first option, or right after -b option (optionally " + "as a range of start and end values).");
			System.exit(-1);
		}
	}
	...
	public static void registerForReset(String className) {
		...
		try {
			...
			System.err.println("Can't register class " + className + " for resetting; class not found");
			System.exit(-1);

		} catch (NoSuchMethodException e) {
			System.err.println("Can't register class " + className + " for resetting; class doesn't contain resetting method");
			System.exit(-1);
		}
		resetList.add(c);
	}
	...
	private static int[] parseNrofRuns(String arg) {
		...
		try {
			...
			System.err.println("Invalid argument '" + arg + "' for" + " number of runs");
			System.err.println("The argument must be either a single value, " + "or a range of values (e.g., '2:5'). Note that this " + "option has changed in version 1.3.");
			System.exit(-1);
		}
		...
		if (val[0] >= val[1]) {
			System.err.println("Starting run value can't be bigger than the " + "last run value");
			System.exit(-1);
		}
	...
	private static void parseCmdSettings(String arg) {
		...
		if (arg.contains(CMD_SETTING_DELIMITER)) set = arg.split(CMD_SETTING_DELIMITER);
		else set = new String[]{arg};
